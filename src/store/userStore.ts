...
/**
 * UserState uses union type User to show type-level legacy vs new.
...
export interface UserContextValue {
  state: UserState;
  setUser: (user: User, isLegacy?: boolean) => void;
  clearUser: () => void;
}
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const setUser = useCallback((user: User, isLegacy: boolean = false) => {
 ...
 */
export function isUserV1(user: User): user is UserV1 {
