...
const log = createLogger('use-user');
...
export function useUser(): UseUserResult {
  ...
  const fetchUser = useCallback(async () => {
    ...
    try {
      if (ENABLE_V1_API) {
        log.info('Fetching user with v1 API');
        const userV1 = await getCurrentUserV1(userId);
        setUser(fromUserV1(userV1));
      } else {
        log.info('Fetching user with v2 API');
...
export function useUserById(userId: string): UseUserResult {
  ...
  const fetchUser = useCallback(async () => {
    ...
    try {
      ...
      setUser(userData);
      log.info('User loaded by ID', { userId });
    } catch (err) {
      log.error('Failed to load user by ID', { userId });
      setError(err instanceof Error ? err : new Error('Failed to load user'));
    } finally {
      setLoading(false);
    }
